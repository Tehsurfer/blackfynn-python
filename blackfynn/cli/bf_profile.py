...
def main():
    ...
    if not os.path.exists(settings.config_file):
        ...
        create_profile(settings, args['<args>'])
    elif args['<command>'] == 'delete':
        delete_profile(settings, args['<args>'], args['--force'])
    elif args['<command>'] == 'set-default':
        set_default(settings, args['<args>'])
    elif args['<command>'] == 'list' or args['--contents']:
        list_profiles(settings, args['--contents'])

    elif args['<command>'] == 'show':
        show_profile(settings, args['<args>'])

    elif args['<command>'] == 'set':
        set_key(settings, args['<args>'], args['--force'])
    elif args['<command>'] == 'unset':
        unset_key(settings, args['<args>'], args['--force'])
...
def create_profile(settings, name=[]):
    ...
    if name in settings.config:
        ...
        if settings.config['global']['default_profile'] == 'none':
            ...
            set_default = input("Would you like to set '{}' as default (Y/n)? ".format(name, settings.config['global'][
                'default_profile'])).lower() in ['y', 'yes']
            if set_default:
                settings.config['global']['default_profile'] = name
                print(("Default profile: {}".format(name)))
        ...
        for key, value in sorted(settings.profiles[name].items()):
            if key != 'default_profile':
                if key in settings.config[name] and name != 'global':
                    print((' ' * indent + '{:{key_len}} : \033[32m{}\033[0m ({})'.format(key, value, name,
                                                                                         key_len=key_len)))
                elif key in settings.config['global']:
                    print((' ' * indent + '{:{key_len}} : \033[34m{}\033[0m (global)'.format(key, value,
                                                                                             key_len=key_len)))
                else:
